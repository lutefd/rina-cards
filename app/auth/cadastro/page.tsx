...
export default function CadastroPage() {
	...
	const handleSignUp = async (e: React.FormEvent) => {
		...
		try {
			// Use our custom sign-up method - all users register as customers
			const { success, error } = await customAuth.signUp({
				name: nomeCompleto,
				email,
				password,
				userType: "customer",
				...
				<div className="flex flex-col gap-6">
					<div className="flex flex-col items-center gap-2 text-center">
						...
						<p className="text-sm text-muted-foreground">
							Marketplace de Photocards K-pop
						</p>
					</div>
					<Card>
						<CardHeader>
						...
						<CardContent>
							<form onSubmit={handleSignUp}>
								...
								<div className="mt-4 text-center text-sm">
									...
									<Link
										href="/auth/login"
										className="underline underline-offset-4 text-pink-600 hover:text-pink-700"
									>
										Entrar
									</Link>
								</div>
