...
export function FazerPedidoDialog({
	cegId,
	photocards,
}: FazerPedidoDialogProps) {
	...
	const updateQuantidade = (photocardId: string, quantidade: number) => {
		...
		setSelectedPhotocards((prev) =>
			prev.map((p) => (p.id === photocardId ? { ...p, quantidade } : p))
		);
	};
			...
			<DialogContent className="max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
				<DialogHeader>
					...
					<DialogDescription className="text-xs md:text-sm">
						Selecione os photocards que deseja encomendar e preencha suas
				...
				<form
					...
					className="space-y-4 md:space-y-6 flex-1 overflow-y-auto"
				>
					{/* Seleção de Photocards */}
					<div>
						<div className="flex items-center justify-between mb-3">
							<Label className="text-sm md:text-base font-semibold">
								Selecione os Photocards
							</Label>
							<span className="text-xs md:text-sm text-muted-foreground">
								{selectedPhotocards.length} selecionado
								{selectedPhotocards.length !== 1 ? "s" : ""}
							</span>
						</div>
						...
						<div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2 md:gap-3 max-h-[40vh] overflow-y-auto border rounded-lg p-2 md:p-3">
							{filteredPhotocards.length > 0 ? (
								filteredPhotocards.map((pc) => {
									const selected = selectedPhotocards.find(
										(s) => s.id === pc.id
									);
									...
									return (
										<div
											key={pc.id}
											onClick={() => !isOutOfStock && togglePhotocard(pc.id)}
											className={`border rounded-lg p-1.5 md:p-2 transition-all ${
												isOutOfStock
													? "opacity-50 cursor-not-allowed bg-gray-100"
													: selected
													? "border-pink-500 bg-pink-50 ring-2 ring-pink-200 cursor-pointer"
										...
										>
											<div className="aspect-3/4 bg-gray-100 rounded mb-1.5 relative overflow-hidden">
												<img
													src={
														pc.imagem_url ||
														`/placeholder.svg?height=200&width=150&query=${pc.idol} photocard`
													}
													alt={pc.titulo}
												...
												{isOutOfStock && (
													<div className="absolute inset-0 bg-black/40 flex items-center justify-center">
														<span className="bg-gray-800 text-white text-[10px] md:text-xs px-2 py-1 rounded font-medium">
															Esgotado
														</span>
													</div>
												)}
											</div>
											<h4 className="font-medium text-[10px] md:text-xs line-clamp-2">
												{pc.titulo}
											</h4>
							...
							<div className="space-y-2 max-h-[30vh] overflow-y-auto">
								{selectedPhotocards.map((item) => {
									...
									return (
										<div
											key={item.id}
											className="flex items-center gap-2 bg-white p-2 md:p-3 rounded-lg"
										>
											...
											<div className="flex items-center gap-1.5 md:gap-3">
												<div className="flex items-center gap-1 md:gap-2">
													...
													<Button
														...
														disabled={
															photocard.estoque !== null &&
															item.quantidade >= photocard.estoque
														}
													>
														+
													</Button>
												...
												<p className="text-xs md:text-sm font-bold text-pink-600 w-14 md:w-20 text-right">
													R${" "}
													{((photocard.preco || 0) * item.quantidade).toFixed(
														2
													)}
												</p>
												<Button
					...
					<div className="space-y-3 md:space-y-4">
						<div>
							...
							<p className="text-[10px] md:text-xs text-muted-foreground mt-1">
								O vendedor usará este número para entrar em contato
							</p>
						</div>

						<div>
							<Label htmlFor="observacoes" className="text-sm md:text-base">
					...
					<div className="flex flex-col sm:flex-row gap-2 md:gap-3 pt-4 border-t sticky bottom-0 bg-white">
						...
						>
							{isSubmitting
								? "Enviando..."
								: `Confirmar Pedido (${selectedPhotocards.length})`}
						</Button>
					</div>
				</form>
			</DialogContent>
